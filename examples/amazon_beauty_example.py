...
def read_data(path: str) -> DataFrame:
    ...
    data = pd.read_csv(os.path.join(os.path.abspath(path), "data.csv"), names=DATASET_SCHEMA, skiprows=1).assign(
        user_id=lambda x: user_encoder.fit_transform(x["user_id"]),
        item_id=lambda x: item_encoder.fit_transform(x["item_id"]),
    )

    users, user_counts = np.unique(data["user_id"], return_counts=True)
    mask = np.isin(data["user_id"], users[user_counts >= 20])
    return data[mask].sort_values("timestamp", ignore_index=True)
...
@click.command()
@click.option("--input_path", default="data/amazon_beauty")
@click.option("--output_path", default="results/amazon_beauty")
